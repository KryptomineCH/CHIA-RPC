...
namespace CHIA_RPC.ErrorInterface_NS
{
    public class ReportError
    {
        ...
        public static async Task UploadFileAsync(Error error)
        {
            ...
            string baseDir = "/errorlog/chia-rpc-library/";

            // Read Private Key from embedded resources
            Stream keyStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("CHIA_RPC.errorlog");
            ...
            if (FilenameCache.TryGetValue(error.ObjectName + error.Function, out DateTime lastUsed) && (DateTime.Now - lastUsed).TotalDays < 1)
            {
                return; // Skip upload for this filename
            }

            // Update cache
            FilenameCache[error.ObjectName + error.Function] = DateTime.Now;
            ...
            string prefixedFilename = $"{currentUnixTimestampMs}-{error.ObjectName}-{error.Function}";
